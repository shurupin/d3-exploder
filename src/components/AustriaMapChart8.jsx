...
export default class AustriaMapChart8 extends Component {
  constructor(props) {
    super(props);
  ...
  drawExploder = (myExploder) => {
    ...
    d3.json(
      'https://raw.githubusercontent.com/ginseng666/Austria-Hextiles-deformed/master/counties/bezirke_normal.json',
    )
      .then(function (at) {
        const state_features = topojson.feature(at, at.objects.bezirke).features;
        ...
        const states = g
          ...
          .data(
            state_features.filter(function (d) {
              return (
                d.properties.name == 'Braunau am Inn' ||
                d.properties.name == 'Gmunden' ||
                d.properties.name == 'Ried im Innkreis' ||
                d.properties.name == 'Vocklabruck' ||
                d.properties.name == 'Salzburg' ||
                d.properties.name == 'Hallein' ||
                d.properties.name == 'Salzburg-Umgebung' ||
                d.properties.name == 'Sankt Johann im Pongau'
              );
            }),
          )
          .enter()
        ...
        function addButton(text, callback) {
          d3.select('#buttons-austria-map-chart-8')
            .append('button')
            .text(text)
            .on('click', function () {
  ...
  render() {
    return (
      <>
        <div id="buttons-austria-map-chart-8" />
        <div id="austria-map-chart-8" ref={this.mapRef} />
      </>
    );
  }
}
